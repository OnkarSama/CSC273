...
/**
 * SortedLinkedDictionary.java - implementation of a linked-based sorted Dictionary ADT
 *
 * @author (your name)
        ...
        implements DictionaryInterface<K,V>
{
    ...
    public V add(K key, V value)
    {
        ...
        //keep going as long as the given key comes after the current node's key
        while ((currentNode != null) && (key.compareTo(currentNode.getKey()) > 0))
        {
            nodeBefore = currentNode;       // catch up nodeBefore
            currentNode = currentNode.next; // advance currentNode to next link
        }

        //Is the key already in the dictionary?
        if (currentNode != null && key.equals(currentNode.getKey()))
        {
            //replace old value with the new given value
            result = currentNode.getValue();
            currentNode.setValue(value);
        }
        //otherwise, need to add as a new entry
        else {
            Node newNode = new Node(key, value);

            //are we adding to the front of the dictionary?
            if (currentNode == firstNode)
            {
                newNode.next = firstNode;
                firstNode = newNode;
            } //otherwise, add new node in between nodeBefore and currentNode
            else {
                nodeBefore.next = newNode;
                newNode.next = currentNode;
            }

            numberOfEntries++;  //either way we added an entry
        }
    ...
    public V remove(K key)
    {
        ...
        //keep going as long as given key comes after the current node's key
        while ((currentNode != null) && (key.compareTo(currentNode.getKey()) > 0))
        {
